 ...
 * localStorage.access_token have been removed because:
 *   1. localStorage is accessible to any JavaScript on the page (XSS risk).
 *   2. HttpOnly cookies achieve the same goal — keeping the token available
 *      on every request — without the XSS exposure.
 *
 ...
 * access_token if the refresh_token is still valid.
 */
...
export const authApi = {
  ...
  login: (data: { email: string; password: string }) =>
    isMockApiEnabled() ? mockHandlers.login(data) : api.post('/api/auth/login', data),
...
export const authApi = {
   ...
   * Credentials are sent via HttpOnly cookies automatically.
   */
  updateProfile: (data: {
    name?: string;
